...
fn test_roundtrip_with_expected(name: &str, data: Value, expected: Value) {
    ...
    assert_eq!(
        ...
        name, data, expected, decompressed
    );
}
...
fn test_nested_object_with_one_key() {
    test_roundtrip("nested object with one key", json!({
...
fn test_nested_object_with_same_key() {
    test_roundtrip("nested object with same key", json!({
...
fn test_nested_object_and_array_with_same_key() {
    test_roundtrip("nested object and array with same key", json!({
        "Triangles": { "Triangles": [{ "Name": "Triangle-01" }] }
    }));
}
...
fn test_single_key_object_using_existing_value_as_key() {
    test_roundtrip("Single-key object using existing value as key", json!({
        "Name": "Start",
        "Triangles": {
            "Name": "Triangle-01"
        }
...
fn test_array_using_existing_values_used_by_object_key() {
    test_roundtrip("Array using existing values used by object key", json!({
        "obj": {
            "id": 1,
            "name": "arr"
        },
...
fn test_object_prototype_conflicts() {
    test_roundtrip("Handle name conflict with Object.prototype", json!({
        "toString": 1,
        "valueOf": 2,
        "hasOwnProperty": 3,
        "constructor": 4,
...
fn test_string_as_key_and_value() {
    test_roundtrip("A string appears as both key and value", json!({
        "any-1": {
            "key-and-value": "any-3"
        },
        "any-2": "key-and-value"
...
fn test_invalid_numbers_become_null() {
    ...
    test_roundtrip("array with nulls for invalid numbers", json!([null, null, null]));
}
...
fn test_deeply_nested_structure() {
    test_roundtrip("deeply nested structure", json!({
        "a": {
            "b": {
                "c": {
                    "d": {
...
fn test_mixed_array_types() {
    test_roundtrip("mixed array types", json!([
        1,
        "string",
        true,
        false,
...
fn test_unicode_strings() {
    test_roundtrip("unicode strings", json!({
        "emoji": "🎉🚀💻",
        "chinese": "中文测试",
        "japanese": "日本語テスト",
...
fn test_special_characters_in_strings() {
    test_roundtrip("special characters in strings", json!({
        "newline": "line1\nline2",
        "tab": "col1\tcol2",
        "quote": "say \"hello\"",
...
fn test_large_integers() {
    test_roundtrip("large integers", json!({
        "small": 1,
        "medium": 1000000,
...
fn test_boolean_values() {
    test_roundtrip("boolean values", json!({
        "true_val": true,
        "false_val": false,
        "arr": [true, false, true]
    }));
...
fn test_null_values_in_objects() {
    test_roundtrip("null values in objects", json!({
        "present": "value",
        "absent": null,
        "nested": {
            "inner_present": 42,
...
fn test_escaped_prefix_strings() {
    ...
    test_roundtrip("escaped prefix strings", json!({
...
fn test_repeated_keys_in_arrays_of_objects() {
    ...
    test_roundtrip("repeated keys in arrays of objects", json!([
        { "id": 1, "name": "first" },
        { "id": 2, "name": "second" },
        { "id": 3, "name": "third" }
    ]));
...
fn test_complex_real_world_like_data() {
    test_roundtrip("complex real-world-like data", json!({
        "users": [
            {
                "id": "user-001",
                "profile": {
